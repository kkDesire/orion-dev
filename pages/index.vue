...
import type { Category, Module } from '~/server/utils/drizzle'
...
if (filter && categories.value.length && categories.value.some((category: Category) => category.slug === filter))
...
if (filter && modules.value.length && modules.value.some((module: Module) => module.slug === filter))
  ...
  <div>
    <ULandingHero>
      ...
      <template #description>
        A community-driven collection of template for your next project, <br>from
        landing page to complete web application.
      </template>
    </ULandingHero>
    <section class="my-2 py-12 ring-1 dark:ring-gray-800 dark:bg-gray-800 dark:bg-opacity-20">
      <div class="px-4 max-w-screen-lg mx-auto flex flex-col items-center md:flex-row md:justify-around gap-12">
        <p class="flex flex-col gap-4 max-w-56">
          ...
          <span class="dark:text-gray-400"> Templates available and ready to use accross </span>
        </p>
        <p class="flex flex-col gap-4 max-w-56">
          <span class="text-4xl md:text-5xl font-semibold">34</span>
          <span class="dark:text-gray-400"> Creators share their creations to help you start your project </span>
        </p>
      </div>
    </section>
    <UContainer class="py-24 sm:py-32 md:py-40 flex flex-col md:flex-row gap-6 md:gap-8">
          ...
          <div class="flex flex-col gap-6 md:gap-4">
            ...
            >
              <template #option="{ option }">
                ...
                <template v-else-if="option.value === 'premium'">
                  <UBadge
                    color="blue"
                    variant="subtle"
                  >
      ...
      <div class="w-full flex flex-col gap-6">
        <!-- Actions Toolbar -->
        <div class="flex flex-row gap-4 md:gap-none justify-between">
          <UInput
